...
export function useAppData(user, profile) {
  ...
  const createPost = useCallback(async (content, sessionId = null, mediaFile = null) => {
    if (!uid) return
    ...
    // Upload photo/video to Supabase Storage first (if attached)
    let media_url  = null
    let media_type = null
    if (mediaFile) {
      ...
      if (upErr) { showToast('Failed to upload media', 'error'); return }
      media_url = supabase.storage.from('post-media').getPublicUrl(path).data.publicUrl
    }

    const { error } = await supabase
      .from('posts')
      .insert({ user_id: uid, content, session_id: sessionId || null, media_url, media_type })
